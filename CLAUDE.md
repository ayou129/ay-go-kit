...
3. **检查一致性**：
   - 包之间的依赖关系是否合理（database 不依赖 ginx，ginx 可依赖 database）
   - 导出的 API 命名是否统一
   - 是否有重复或冲突的功能
4. **向用户报告**审计结果，等待确认后再进入第二步
...
- repo 层仍使用 `(ctx, db *gorm.DB, ...)` 显式传参 → 目标：去掉 db 参数，用 `database.GetDB(ctx)`
- service 层混合包函数和 struct → 目标：统一 struct + interface + TxRunner 注入
- 分页用项目自己的 `utils.PageQueryDTO` → 目标：切换到 `ginx.PageQueryDTO` + `database.FindByPage`
- 清洗用项目自己的 `utils.BindJSON` → 目标：切换到 `ginx.BindJSON`
- 密码用项目自己的 `utils.HashPassword` → 目标：切换到 `cryptox.HashPassword`

### flowi（待迁移）

- repo 层使用 `database.BaseRepository`（已废弃）→ 目标：改用 `database.GetDB(ctx)` 直接调用
- 分页手动 `parsePage()` → 目标：切换到 `ginx.PageQueryDTO` + `database.FindByPage`
...
| 场景 | go-kit API |
|------|-----------|
| 获取 DB 连接 | `database.GetDB(ctx)` |
| 事务 | `database.TxRunner` 接口，`database.NewTxRunner()` |
| 分页查询 | `database.FindByPage[T](ctx, pq, scopes...)` |
...
| 记录不存在 | `database.IsRecordNotFound(err)` |
